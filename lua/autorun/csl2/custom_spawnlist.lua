...
concommand.Add( "custom_spawnlist_reloadvgui_propsvn", function()
...
function spawnmenu.AddCustomSpawnlistNode( name, folder, icon, options, skin )
...
local function addCustomContent( self, pnlContent, contents )
    ...
    self.ContentsPanel = vgui.Create( "PropSVN_ContentContainer", pnlContent )
...
local function populateCustomSpawnlists( pnlContent, tree, node )
    ...
    for _, data in pairs( customSpawnlists ) do
        ...
        -- add node
        local custom = tree:AddNode( data.name, data.icon, data.skin )
        ...
        for _, path in pairs( files ) do
            ...
            local newNode = custom:AddNode( info.name, info.icon, info.skin )
			...
			if not data.skin then continue end
            for k, value in pairs( data.skin ) do
                newNode[k] = value
            end
        end
    end
...
hook.Add( "PopulateContent", "populate_custom_spawnlists_vmt_propsvn", populateCustomSpawnlists )
